...
<div class="max-w-5xl mx-auto py-8 px-4">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="phone" value="{{ old('phone', $customer->phone) }}" 
                                   pattern="[267]\d{3}-?\d{4}" 
                                   title="Formato requerido: ####-#### (Debe iniciar con 2, 6 o 7)"
    ...
    <div class="mt-12 bg-red-50/50 rounded-3xl border-2 border-dashed border-red-100 p-8 flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <form action="{{ route('salesperson.clientes.destroy', ['cliente' => $customer->id]) }}" method="POST" onsubmit="return confirm('¿Confirmas la eliminación permanente?')">
            @csrf
            @method('DELETE')
            <button type="submit" class="bg-white text-red-600 border border-red-200 px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all">
                Eliminar Registro
            </button>
        </form>
    </div>
